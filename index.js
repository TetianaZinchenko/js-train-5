...
function printStrings(array) {
  ...
  const combinedString = array.toString();
  // const result = combinedString + combinedString;
  const result = combinedString.concat(combinedString);
  console.log(result);
}
...
function concatenateStrings(array, subString) {
  ...
  const result = mainString.includes(subString);
...
function getEvenNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] % 2 === 0) {
      evenNumbers[evenNumbers.length] = arr[i];
    }
  }
